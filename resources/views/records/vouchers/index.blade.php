    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-6">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Voucher Records') }} ({{ \App\Models\Voucher::count() }})
            </h2>
            <a href="{{ route('records.vouchers.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Add New Voucher
            </a>
        </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6 bg-white border-b border-gray-200">
                <form action="{{ route('records.vouchers.index') }}" method="GET" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Voucher ID</label>
                            <input type="text" name="voucher_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ request('voucher_id') }}">
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="payee" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ request('payee') }}">
                        </div>
                    </div>
                            ...
                            @forelse($vouchers as $voucher)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $voucher->date->format('M d, Y') }}<br>
                                        {{ $voucher->time->format('h:i A') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $voucher->origin }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ Str::limit($voucher->subject, 50) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $voucher->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800' }}">
                                            {{ ucfirst($voucher->status) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('records.vouchers.show', $voucher) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                                        @if($voucher->status !== 'completed')
                                            <form action="{{ route('records.vouchers.receive', $voucher) }}" method="POST" class="inline">
                                                @csrf
